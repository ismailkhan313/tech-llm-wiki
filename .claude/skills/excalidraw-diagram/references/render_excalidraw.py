...
def render(
    ...
    with sync_playwright() as p:
        ...
        # Wait for the ES module to load (imports the Excalidraw bundle from a CDN)
        try:
            page.wait_for_function("window.__moduleReady === true", timeout=60000)
        except Exception:
            print("ERROR: Excalidraw module never finished loading (CDN unreachable?).", file=sys.stderr)
            browser.close()
            sys.exit(1)

        module_error = page.evaluate("window.__moduleError || null")
        if module_error:
            print(f"ERROR: Could not load the Excalidraw library: {module_error}", file=sys.stderr)
            browser.close()
            sys.exit(1)
