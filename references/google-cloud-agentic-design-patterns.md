...
description: Google Cloud Architecture Center guidance on selecting an agent design pattern, covering single-agent, multi-agent, ReAct, human-in-the-loop, and custom logic patterns.
tags: [source, agents, design-patterns, architecture, google-cloud]
resource: https://docs.cloud.google.com/architecture/choose-design-pattern-agentic-ai-system
author: team:google-cloud
fidelity: verbatim
retrieved: 2026-09-06T04:49:55Z
generated: { by: claude-code/opus-5, at: 2026-09-06T04:49:55Z }
status: stable
---

<!--
VERBATIM prose. Captured from the rendered page; every sentence, heading, list,
and table entry is unmodified. What was dropped is site chrome only: the
breadcrumb nav, the "Send feedback" controls, and the "Stay organized with
collections" popout. Inline HTML anchors were converted to markdown links and
site-relative URLs absolutized against https://docs.cloud.google.com. Diagram
images are referenced at their canonical SVG URLs rather than downloaded, so
they render only with network access; the `resource` URL above is the
authority for the visuals.
Byline: Samantha He (Technical Writer), plus 15 listed contributors.
...
Workflows that are deterministic include tasks that are predictable and sequential, and that have a clearly defined workflow path from start to finish. The steps in your tasks are known in advance, and the process doesn't change much from one run to the next.

The following are agent design patterns for workflows that are deterministic:

[Multi-agent sequential pattern](#sequential-pattern)  
...
[Multi-agent parallel pattern](#parallel-pattern)  
Use this pattern if your workload has the following characteristics:
...
[Multi-agent iterative refinement pattern](#iterative-refinement-pattern)  
Use this pattern if your workload has the following characteristics:
...
Workflows that require dynamic orchestration involve complex problems where agents must decide how to proceed. In these workflows, the agentic AI system dynamically plans, delegates, and coordinates tasks without a script.

The following are agent design patterns for workflows that require dynamic orchestration:

[Single agent pattern](#single-agent-system)  
Use this pattern if your workload has the following characteristics:
...
[Multi-agent coordinator pattern](#coordinator-pattern)  
Use this pattern if your workload has the following characteristics:
...
[Multi-agent hierarchical task decomposition pattern](#hierarchical-task-decomposition-pattern)  
Use this pattern if your workload has the following characteristics:
...
[Multi-agent swarm pattern](#swarm-pattern)  
Use this pattern if your workload has the following characteristics:
...
Workflows that involve iteration include tasks where the final output is achieved through cycles of refinement, feedback, and improvement.
...
[Multi-agent loop pattern](#loop-pattern)  
Use this pattern if your workload has the following characteristics:
...
[Multi-agent review and critique pattern](#review-critique-pattern)  
Use this pattern if your workload has the following characteristics:
...
Workflows that have special requirements include tasks that don't follow common agentic patterns. Your tasks can include unique business logic or require human judgment and intervention at critical points. In this case, your agentic AI system is custom-built for a single, specific purpose.
