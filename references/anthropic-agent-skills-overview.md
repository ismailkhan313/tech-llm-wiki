...
description: Anthropic's Agent Skills overview — why Skills exist, the three levels of progressive disclosure, the Skills architecture, where Skills work across the API, Claude Code, and claude.ai, the SKILL.md structure, security, and the platform limitations.
tags: [source, agent-skills, skills, anthropic, claude-code, progressive-disclosure, context-engineering]
resource: https://platform.claude.com/docs/en/agents-and-tools/agent-skills/overview
author: org:anthropic
fidelity: verbatim
retrieved: 2026-09-07T19:48:41Z
...
path with a `.md` suffix, which is the page's own source rather than a scrape.
The docs' own YAML frontmatter (title, url, description) was replaced by the
OKF block above; its `url` value is preserved as `resource`. Body unmodified,
MDX components (<Note>, <Warning>, <CardGroup>) included.
-->
