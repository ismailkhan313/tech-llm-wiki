...
description: Wikipedia's article on prompt engineering — terminology, the techniques catalog (multi-shot, chain-of-thought, tree-of-thought, text-to-image), model sensitivity, automated prompt generation, limitations, history, and prompt injection.
tags: [source, prompt-engineering, context-engineering, techniques]
resource: https://en.wikipedia.org/w/index.php?title=Prompt_engineering&oldid=1373316427
author: org:wikipedia-contributors
fidelity: excerpt
...
<!--
EXCERPT, not verbatim — the canonical article wins over this copy, and it is a
live page that will have moved on. `resource` above pins the exact revision
this was taken from (oldid 1373316427, last edited 2026-09-05T07:11:33Z), so
the state captured here stays checkable.

Prose is unmodified. Dropped: the infobox, all tables and figures, inline
citation superscripts, and the entire References/External links apparatus
(~250 citations). Section headings are preserved at their original levels.
Text licensed CC BY-SA 4.0 by the Wikipedia contributors.
-->
...
The benefits of chain-of-thought prompting vary considerably by task type. A meta-analysis covering more than 100 studies of CoT found that it delivers large gains primarily on mathematical, logical and symbolic reasoning problems, with much smaller improvements on other kinds of task, and concluded that CoT can be applied selectively in order to reduce inference costs. On tasks drawn from cognitive psychology in which deliberation is known to impair human performance, CoT has been found to reduce the accuracy of state-of-the-art models, in some cases substantially. A systematic review of structured prompting techniques similarly reports that the additional reasoning steps increase response times and interaction complexity, so that the technique is not uniformly preferable to simpler prompts for factual or low-complexity queries.
...
- See also: Artificial intelligence visual art § Prompt engineering and sharing, and Artificial intelligence visual art
...
'''Top''': no negative prompt<br>'''Centre''': \"green trees\"<br>'''Bottom''': \"round stones, round rocks\""}},"i":0}}]}' id="mwAVM"> Demonstration of the effect of negative prompts on images generated with Stable DiffusionTop: no negative promptCentre: "green trees"Bottom: "round stones, round rocks"
...
Early precedents of structured natural-language interaction with rule-based systems can be traced to ELIZA, a 1966 program by Joseph Weizenbaum that used pattern matching and scripted substitution rules to simulate conversation, most famously imitating a Rogerian psychotherapist. Although ELIZA did not involve neural networks or learned language models, its use of keyword-triggered rules to shape a program's output anticipated later human-in-the-loop approaches to guiding AI systems through carefully structured input.
