...
description: Anthropic's Skill authoring guide — conciseness, degrees of freedom, naming and descriptions, progressive-disclosure patterns, workflows and feedback loops, content guidelines, evaluation-driven development, anti-patterns, guidance for Skills with executable code, and the pre-share checklist.
tags: [source, agent-skills, skills, anthropic, best-practices, evaluation, progressive-disclosure]
...
path with a `.md` suffix. The docs' own YAML frontmatter (title, url,
description) was replaced by the OKF block above; its `url` value is preserved
as `resource`. Body unmodified, MDX components included.
-->
