...
description: The MCP documentation's architecture page — scope, participants (host, client, server), the data and transport layers, statelessness and discovery, the server and client primitives, notifications, and a full JSON-RPC worked example.
tags: [source, mcp, protocols, architecture, json-rpc, primitives]
resource: https://modelcontextprotocol.io/docs/2026-07-28/learn/architecture
author: org:model-context-protocol
fidelity: verbatim
retrieved: 2026-09-07T07:40:00Z
generated: { by: claude-code/opus-5, at: 2026-09-07T07:40:00Z }
status: stable
---

<!--
VERBATIM. Captured from the markdown source the docs site serves at the same
path with a `.md` suffix, which is the page's own source rather than a
scrape. Documentation version 2026-07-28 — note this revision deprecates the
sampling and logging client primitives and replaces the initialize handshake
with a stateless `server/discover` request, so it differs from older accounts
of MCP.

Unmodified except: the site-injected "Documentation Index" blockquote at the
top was dropped. Mintlify MDX components (<Note>, <Tooltip>, <Steps>, <Step>,
<CodeGroup>) and the mermaid block are left exactly as published; relative
links resolve against https://modelcontextprotocol.io.
-->
