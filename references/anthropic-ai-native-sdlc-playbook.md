...
description: Anthropic's Applied AI team's playbook for rebuilding the software development lifecycle around agentic coding — the six stages, twelve plays, the committed-artifact chain, and the governance controls for each stage. All fourteen lessons of the Claude Academy course.
tags: [source, sdlc, claude-code, anthropic, agentic-coding, governance, hooks, skills, evals, ci-cd, enterprise]
resource: https://academy.claude.com/courses/ai-native-sdlc-playbook
author: org:anthropic
fidelity: verbatim
retrieved: 2026-09-08T00:00:00Z
generated: { by: claude-code/opus-5, at: 2026-09-08T00:00:00Z }
status: stable
---

<!--
VERBATIM PROSE. All fourteen lesson pages of the Claude Academy course "The
AI-Native SDLC Playbook," concatenated in course order under the six stage
headings the course groups them by. Each lesson keeps its own source URL.

Transformations, none of which touch the prose: the pages are server-rendered
HTML, converted here with pandoc from each lesson's `<article>` element; site
chrome, heading anchor links, icon glyphs, and the code blocks' "copy" buttons
were dropped; each lesson's headings were demoted one level so the whole course
nests under this file's structure; the three figures are referenced by their
absolute URL on academy.claude.com with their alt text and caption intact;
"(opens in new tab)" suffixes were stripped from link text; and the one prompt
the course renders as a copyable block was fenced as code.
-->
...
- [Set up Claude Code for your organization](https://code.claude.com/docs/en/admin-setup) (the admin decision map; start here)
...
- [Plugins and private marketplaces](https://code.claude.com/docs/en/plugin-marketplaces) (how skills and hooks are distributed organization-wide)
- [Managed MCP](https://code.claude.com/docs/en/managed-mcp) (central control of the agent's tool surface)
- [Enterprise deployment overview](https://code.claude.com/docs/en/third-party-integrations) (Amazon Bedrock, Vertex AI, Microsoft Foundry)
- [Enterprise network configuration](https://code.claude.com/docs/en/network-config)
- [Monitoring (OpenTelemetry)](https://code.claude.com/docs/en/monitoring-usage) and [the analytics dashboard](https://code.claude.com/docs/en/analytics)
- [Compliance API](https://platform.claude.com/docs/en/manage-claude/compliance-api) (Enterprise activity feed, chat retrieval and deletion)
